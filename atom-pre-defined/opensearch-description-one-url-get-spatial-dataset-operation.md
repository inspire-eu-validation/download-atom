...
The OpenSearch description must contain a ["Url" element](#urlelement) that describes a template URL for the Get Spatial Data Set operation. This template must accept the INSPIRE parameters "crs", "spatial_dataset_identifier_code" and the OpenSearch "language" parameter. This template may also accept the optional INSPIRE parameter "spatial_dataset_identifier_namespace" if the dataset identifier has a namespace component. The ["Url" element](#urlelement) must have an attribute "type" with a value corresponding to the media type of the result and an attribute "rel" with the value "results".
...
* for each [Get Spatial Dataset url](#getspatialdataseturl), fill in the parameters for "crs", "spatial_dataset_identifier_code", "spatial_dataset_identifier_namespace" (if present) and "language" and retrieve the resource at the [Get Spatial Dataset url](#getspatialdataseturl)
...
* [TG DL](./README.md#ref_TG_DL), Requirement 43
...
The values for the parameters "crs", "spatial_dataset_identifier_code", "spatial_dataset_identifier_namespace" (if present) and "language" can be obtained from the OpenSearch description
